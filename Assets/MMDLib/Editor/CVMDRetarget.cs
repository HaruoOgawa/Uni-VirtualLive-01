...
namespace mmdlib
{
    ...
    public class CVMDRetarget : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // PMXのプレファブ
            m_PmxObj = EditorGUILayout.ObjectField(m_PmxObj, typeof(GameObject), false);

            // ボタンを追加
            if (GUILayout.Button("ReTarget BoneAnimation"))
            {
                ReTargetBoneAnimation(target, m_PmxObj);
            }

            // ボタンを追加
            if (GUILayout.Button("ReTarget BlendShape"))
            {
                ReTargetBlendShapeAnimation(target, m_PmxObj);
            }
        }

        void ReTargetBoneAnimation(Object target, Object pmxObj)
        {
        ...
        void ReTargetBlendShapeAnimation(Object target, Object pmxObj)
        {
            // CurveのRalativePathをリターゲットした新しいアニメーションクリップを作成
            if (target != null && pmxObj != null)
            {
                ...
                SkinnedMeshRenderer[] renderers = pmxGameObject.GetComponentsInChildren<SkinnedMeshRenderer>();
                if (renderers.Length == 0)
                {
                    Debug.LogErrorFormat("Failed to BlendShape Retarget. No SkinnedMeshRenderer!!");
                    return;
                }

                Transform meshTrans = renderers[0].transform;

                string NewMeshPath = meshTrans.name;

                Transform currentParentTrans = meshTrans.parent;

                while (currentParentTrans != null && currentParentTrans != pmxGameObject.transform)
                {
                    NewMeshPath = currentParentTrans.name + "/" + NewMeshPath;

                    currentParentTrans = currentParentTrans.parent;
                }
                ...
                foreach (var binding in CurveBindings)
                {
                    ...
                    DstClip.SetCurve(NewMeshPath, binding.type, binding.propertyName, curve);
                }
                ...
                Debug.LogFormat("ReTarget BlnedShape {0} to {1}.", SrcClip.name, pmxGameObject.name);
            }
        }
